...
def create_shape_mask(size, shape_type):
    ...
    if shape_type == '圓形':
        ...
        d = r * math.sqrt(2)
        draw.rectangle((cx-d/2, cy-d/2, cx+d/2, cy+d/2), fill=255)
    elif shape_type == '正三角形':
        draw.polygon(get_poly_points(3), fill=255)
    elif shape_type == '正五邊形':
        draw.polygon(get_poly_points(5), fill=255)
    elif shape_type == '正六邊形':
        draw.polygon(get_poly_points(6), fill=255)
...
def create_gradient_image(size, start_hex, end_hex, angle):
    ...
    left = (diag - w) // 2
    top_y = (diag - h) // 2
    return res.crop((left, top_y, left+w, top_y+h))
...
def get_color_match_mask(img_rgba, target_color_hex, tolerance=40):
    try:
        ...
        diff = ImageChops.add(
            ImageChops.difference(r, Image.new('L', r.size, tr)),
            ImageChops.add(
                ImageChops.difference(g, Image.new('L', g.size, tg)),
                ImageChops.difference(b, Image.new('L', b.size, tb))
            )
        )
...
def process_single_image_fill(img, opaque_sets, trans_sets, semi_sets, bg_sets, crop_sets):
    ...
    def proc_layer(base, region_mask, s):
        ...
        if tm in ['specific', 'non_specific']:
            ...
            if tm == 'specific': final_mask = ImageChops.multiply(region_mask, cm)
            else: final_mask = ImageChops.multiply(region_mask, ImageChops.invert(cm))
        ...
        if fmod == 'color':
            fill_layer = Image.new('RGBA', (w,h), s.get('fill_color'))
        elif fmod == 'gradient':
            gset = s.get('fill_gradient', {})
            fill_layer = create_gradient_image((w,h), gset.get('start'), gset.get('end'), gset.get('angle', 0))
        elif fmod == 'image' and os.path.exists(s.get('fill_image_path', '')):
            try:
                fi = Image.open(s.get('fill_image_path')).convert('RGBA')
                fi = fi.resize((w,h))
                fill_layer = fi
            except: pass
    ...
    if bg_sets and bg_sets.get('enabled'):
        bg_layer = None
        bt = bg_sets.get('material_type')
        if bt == 'color':
            bg_layer = Image.new('RGBA', (w,h), bg_sets.get('color'))
        elif bt == 'gradient':
            g = bg_sets.get('gradient', {})
            bg_layer = create_gradient_image((w,h), g.get('start'), g.get('end'), g.get('angle', 0))
        elif bt == 'image' and os.path.exists(bg_sets.get('image_path', '')):
            try:
                bi = Image.open(bg_sets.get('image_path')).convert('RGBA')
                bg_layer = Image.new('RGBA', (w,h))
                bg_layer.paste(bi.resize((w,h)))
            except: pass
        ...
        if bg_layer:
            ...
            if mode == 'overlay':
                ...
                ct = bg_sets.get('cutout_target')
                cut_mask = None
                if ct == 'opaque': 
                    cut_mask = img.split()[3].point(lambda x: 255 if x > 200 else 0)
                elif ct == 'transparent':
                    cut_mask = img.split()[3].point(lambda x: 255 if x < 10 else 0)
                elif ct == 'color':
                    cut_mask = get_color_match_mask(img, bg_sets.get('cutout_color'), 30)
                
                if cut_mask:
                    bg_layer.putalpha(ImageChops.multiply(bg_layer.split()[3], ImageChops.invert(cut_mask)))
    ...
    if crop_sets:
        ...
        if shape and shape != '無':
            shape_mask = create_shape_mask((w,h), shape)
            final.putalpha(ImageChops.multiply(final.split()[3], shape_mask))
            
        if crop_sets.get('trim'):
            bbox = final.getbbox()
            if bbox:
...
def task_image_fill(log_callback, progress_callback, current_file_callback, file_progress_callback,
    ...
    for i, fp in enumerate(files):
        try:
            ...
            current_file_callback(fp.name)
            file_progress_callback(0)
            
            rel = fp.relative_to(Path(input_path)) if Path(input_path).is_dir() else fp.name
            dest = out_base / rel.parent
    ...
    current_file_callback("Done")
    file_progress_callback(100)
    log_callback("🏁 結束")

def task_scaling(log_callback, progress_callback, current_file_callback, file_progress_callback,
                 input_path, output_path, mode, mode_value_1, recursive, convert_jpg, lower_ext,
                 delete_original, prefix, postfix, crop_doubao, 
                 sharpen_factor, brightness_factor, 
                 remove_metadata, author, description):
    
    log_callback(f"🚀 [Scaling] 開始")
    files = get_files(input_path, recursive, file_types='image')
    total = len(files)
    out_base = Path(output_path)
    out_base.mkdir(parents=True, exist_ok=True)
    ...
    for i, fp in enumerate(files):
        ...
        try:
            ...
            with Image.open(fp) as img:
                ...
                w, h = img.size
                nw, nh = w, h
                if mode == 'ratio' and mode_value_1 != 1:
                    nw, nh = int(w*mode_value_1), int(h*mode_value_1)
                elif mode == 'width' and mode_value_1 > 0:
                    r = mode_value_1 / w
                    nw, nh = int(mode_value_1), int(h*r)
                elif mode == 'height' and mode_value_1 > 0:
                    r = mode_value_1 / h
                    nh, nw = int(mode_value_1), int(w*r)
...
def task_video_sharpen(log_callback, progress_callback, current_file_callback, file_progress_callback,
                       input_path, output_path, recursive, lower_ext, delete_original, prefix, postfix,
                       luma_m_size, luma_amount, scale_mode, scale_value, convert_h264,
                       remove_metadata, author, description):
    ...
    for fp in files:
        try:
            ...
            new_stem = f"{prefix}{fp.stem}{postfix}"
            final_ext = ".mp4" if convert_h264 else fp.suffix
            if lower_ext: final_ext = final_ext.lower()
            out_file = dest / f"{new_stem}{final_ext}"
            ...
            if scale_mode == 'ratio' and scale_value != 1:
                ...
                px = 1080 if scale_mode == 'hd1080' else 720
...
def task_rename_replace(log_callback, progress_callback, current_file_callback, file_progress_callback,
                        input_path, recursive, do_prefix, old_prefix, new_prefix,
                        do_suffix, old_suffix, new_suffix,
                        remove_metadata, author, description):
    
    log_callback("🚀 [Rename] 開始")
...
def task_multi_res(log_callback, progress_callback, current_file_callback, file_progress_callback,
    ...
    log_callback("🚀 [Icon] 開始")
    sizes = [1024, 512, 256, 128, 64, 32]
    files = get_files(input_path, recursive, file_types='image')
    out_base = Path(output_path)
    
    for i, fp in enumerate(files):
        progress_callback(int((i/len(files))*100))
        current_file_callback(fp.name)
        try:
            with Image.open(fp) as img:
                w, h = img.size
                ref = w if orientation == 'h' else h
                dest = out_base / (fp.relative_to(Path(input_path)).parent if Path(input_path).is_dir() else fp.parent.name)
                dest.mkdir(parents=True, exist_ok=True)
                for s in sizes:
                    if ref >= s:
